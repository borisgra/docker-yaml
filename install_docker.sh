#!/bin/bash
# FROM https://raw.githubusercontent.com/Jigsaw-Code/outline-server/master/src/server_manager/install_scripts/install_server.sh
...
function main() {
  ...
  parse_flags "$@"
#  install_shadowbox #boris.gra
  run_step "Verifying that Docker is installed" verify_docker_installed #boris.gra
  run_step "Verifying that Docker daemon is running" verify_docker_running #boris.gra
}
